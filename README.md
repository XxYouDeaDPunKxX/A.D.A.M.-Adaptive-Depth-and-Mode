...
✨ What you get
- 🏷️ A visible "checksum" on replies through the mode contract
- 🎛️ Manual override when you want it (`LOW`, `MID`, `DEEP` as first token)
- 🚪 DEEP by consent (gated; enters only after `yes` exact, or manual `DEEP`)
- 🧷 Explicit bootstrap state through `BOOTSTRAP_CLASS`
- 🛠️ Tool/asset workflows (non-normative): A.D.A.M. can improve prompt clarity, iteration discipline, and stop criteria (e.g., image generation). It does not "improve the renderer".
...
6. ✍️ Paste your task or artifact.
...
- 🛡️ **Publish-boundary hardening (OVL)**: format-only validation at the publish boundary (draft -> validate -> rewrite once -> fail-closed).
...
- 🏗️ A.D.A.M. assumes the host supports a real system prompt (or equivalent "instructions").
...
## ⚙️ Core Behavior (runtime contract)
