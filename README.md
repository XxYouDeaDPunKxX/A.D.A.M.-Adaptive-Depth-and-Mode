...
A spec-first control protocol for deterministic depth switching in AI conversations.
...
Positioning (SSOT)
Behavioral protocol -> deterministic depth switching + bounded audit -> for high-signal, decision-grade chat work.
...
Note: Some hosts provide upload metadata only (e.g., `<uploaded_files>...</uploaded_files>` with a path) and keep the file contents out-of-band. In that case, A.D.A.M. may respond with `ADAM_UPLOAD_META_ONLY`; you must send `ADAM PING`/`ADAM ON` (or paste the Guest Card) to activate deterministically.
Note: `ADAM_UPLOAD_META_ONLY` is intentionally conservative (whitelist-only). If it does not trigger, use `ADAM PING` anyway.
Example: `ADAM_UPLOAD_META_ONLY` means the upload was classified, not that activation is complete. A deterministic next step is: send `ADAM PING`, then expect `ADAM_PING_OK`.
...
A.D.A.M. is a spec-first control protocol. v4 is an architectural refactor toward determinism: structural-only routing (form, not language), consent-gated DEEP, bounded auditability, and publish-boundary format validation.
...
- **Host-robust recovery**: `RELOAD KERNEL` re-injects a minimal remount header; anchors help detect truncation in short-context hosts.
...
These signals are now codified as deterministic invariants (State Overlay Lite + bounded `AUDIT` + decision-complete): [`tests/state_overlay_lite.md`](tests/state_overlay_lite.md)
...
The other major shift is that you discover sooner that many failures are not "the spec's fault" but the host's. Short context windows, uploads treated as reference rather than system, UI banners pushing the first line, lazy-loaded history: A.D.A.M. cannot remove those constraints. It can only do two sane things: fail-closed when the contract cannot be reliably satisfied, and provide deterministic recovery (operator procedure + probes). That changes the mindset from "hope the model remembers" to "design for unreliable transport."

There is also a trade-off that is easy to miss: creativity and determinism can coexist, but they often require a two-phase workflow. Open exploration is not something A.D.A.M. should slowly absorb by becoming more permissive. Its job is to converge, decide, and verify. So the right UX move is not "make the core looser," it is "reduce friction for switching regimes and make recovery instant when the host drifts."
...
- Deterministic fixed strings (tags, control commands, probes, hard-stops).
...
- `RELOAD KERNEL` -> hard remount: outputs the strict multi-line `KERNEL_REMOUNT v4` block (re-injects minimal SSOT header into chat context)
...
| Kernel reload | `RELOAD KERNEL` | hard remount: outputs the strict multi-line `KERNEL_REMOUNT v4` block |
